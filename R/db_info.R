...
#' Decompress chunks for a hypertable
#'
#' Manually decompress chunks for a hypertable.
#'
#' @param table_name Name of the hypertable
#' @param newer_than Time interval to decompress chunks newer than this (optional)
#' @param conn Database connection object (optional, uses global connection if NULL)
#' @param debug Enable debug messages (optional, default FALSE)
...
#' decompressed <- decompress_chunks("sensor_data")
#' 
#' # Decompress chunks newer than 1 day
#' decompressed <- decompress_chunks("sensor_data", "1 day")
#' 
#' # Or with explicit connection
#' conn <- get_connection()
#' decompressed <- decompress_chunks("sensor_data", conn = conn)
#' }
decompress_chunks <- function(table_name, newer_than = NULL, conn = NULL, debug = FALSE) {
  if (debug) {
    if (is.null(newer_than)) {
      cat("[DEBUG] Decompressing all chunks for table:", table_name, "\n")
    } else {
      cat("[DEBUG] Decompressing chunks for table:", table_name, "newer than:", newer_than, "\n")
    }
  }
  ...
  tryCatch(
    {
      if (debug) cat("[DEBUG] Executing decompress_chunk command\n")
      ...
      ", params = list(table_name))
        ...
        # Decompress chunks newer than specified time
        result <- DBI::dbGetQuery(conn, "
        SELECT decompress_chunk(chunk_name)
        FROM timescaledb_information.chunks
        WHERE hypertable_name = $1
        AND range_start > NOW() - INTERVAL $2
        AND is_compressed
      ", params = list(table_name, newer_than))
      }
      
      num_decompressed <- nrow(result)
      if (debug) cat("[DEBUG] Decompressed", num_decompressed, "chunks\n")
